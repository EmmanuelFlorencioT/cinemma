<?php
    ...
    $stmt = mysqli_prepare($connection, "SELECT user_id, name, email FROM User WHERE email=? AND password=?");
    ...
    try{
        ...
        if(mysqli_stmt_fetch($stmt)){ //The DB found a user match
            ...
            $admin_stmt = mysqli_prepare($connection, "SELECT admin_id, name, email FROM Admin WHERE email=? AND password=?");
            mysqli_stmt_bind_param($admin_stmt, "ss", $em, $ps);
            mysqli_stmt_execute($admin_stmt);
            mysqli_stmt_bind_result($admin_stmt, $id, $name, $email);
